...
function addStudent(name) {
    students.push({ name: name }); // Aggiungi il nuovo studente all'array students
    updateStudentList();
}

function updateWheel() {
    // Aggiorna la lista dei segmenti della ruota con quelli presenti nell'array students
    wheel.segments = students.map(function(student) {
        return student.name;
    });
    
    // Inizializza e aggiorna la ruota
    wheel.init();
    wheel.update();
    
    console.log('Ruota aggiornata con successo!');
}
...
function showWinnerModal(winnerName) {
  ...
  // Aggiungi il nome del vincitore all'elemento modale
  ...
  var wheel = {
    ...
    onTimerTick: function () {
      ...
      if (finished) {
        ...
        var winnerIndex = wheel.segments.length - Math.floor((wheel.angleCurrent / (Math.PI * 2)) * wheel.segments.length) - 1;
        var winnerName = wheel.segments[winnerIndex];

        // Mostra il nome del vincitore in un modale
    ...
    drawWheel: function () {
      ...
      ctx.lineWidth = 10;
      ctx.strokeStyle = 'rgba(244,0,255,1)';
      ctx.stroke();
    }
  };
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    const jumbotronTextHeight = -750;
  
    jumbotron.addEventListener('mousemove', (e) => {
      const jumbotronRect = jumbotron.getBoundingClientRect();
      const jumbotronWidth = jumbotronRect.width;
      const jumbotronHeight = jumbotronRect.height;
      
      // Verifica che le dimensioni dell'elemento jumbotron siano positive
      if (jumbotronWidth > 0 && jumbotronHeight > 0) {
        const jumbotronCenterX = jumbotronRect.left + jumbotronWidth / 2 - window.scrollX;
        const jumbotronCenterY = jumbotronRect.top + jumbotronHeight / 2 - window.scrollY - (jumbotronTextHeight / 2);
    
        const mouseX = e.clientX - jumbotronCenterX;
        const mouseY = e.clientY - jumbotronCenterY;
        
        const offsetX = mouseX / jumbotronWidth * 2; // Moltiplica per 2 per ottenere un intervallo tra -1 e 1
        const offsetY = mouseY / jumbotronHeight * 2;
        
        const moveAmount = 30; // Regola la velocità del movimento
        
        const translateX = -offsetX * moveAmount + 'px';
        const translateY = (-offsetY * moveAmount + jumbotronTextHeight / 2) + 'px'; // Aggiungi un offset per compensare il testo
